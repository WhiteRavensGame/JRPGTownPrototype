...
namespace Frankfort.VBug.Internal
{
    public class MouseInputProvider : IHumanInputProvider
    {
        ...
        private InputCommand[] GetMouseCommands()
        {
            ...
            for(int i = 0; i < btnMax; i ++)
            {
                ...
                {
                    //Debug.Log("down: " + i);
                    caughtCommands.Add(new InputCommand(id, HumanInputState.down, screenPos));
                }
                else if (Input.GetMouseButtonUp(i))
                {
                   // Debug.Log("up: " + i);
                    caughtCommands.Add(new InputCommand(id, HumanInputState.up, screenPos));
                }
                else if (Input.GetMouseButton(i))
                {
                    // Debug.Log("hold: " + i);
                    caughtCommands.Add(new InputCommand(id, moved ? HumanInputState.downMove : HumanInputState.downHold, screenPos));
                }
                else
                {
                    caughtCommands.Add(new InputCommand(id, moved ? HumanInputState.move : HumanInputState.hold, screenPos));
                }
            } 
            //--------------- Detect states --------------------
            ...
            if (deltaScroll.x != 0f || deltaScroll.y != 0f)
                caughtCommands.Add(new InputCommand("Mouse ScrollWheel", HumanInputState.move, deltaScroll));
