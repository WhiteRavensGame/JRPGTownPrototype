...
namespace Frankfort.VBug.Editor
{
    ...
    public static class VisualResources
    {
        ...
        private static Texture2D iconSpriteSheet;
        private static Texture2D bgWaterMark;

        private static Texture2D xboxSpriteSheet;
        private static Rect xboxBGLoc = new Rect(0,0, 256, 256);
        ...
        public static void Prune()
        {
            if (iconSpriteSheet != null)
                Resources.UnloadAsset(iconSpriteSheet);
            if (bgWaterMark != null)
        ...
        public static Texture2D GetColorTexture(Color color)
        {
            ...
            result = new Texture2D(1, 1, TextureFormat.RGBA32, false, true);
            result.name = "vBug_UIColor";
        ...
        {
            if (iconSpriteSheet == null)
                iconSpriteSheet = (Texture2D)UnityEditor.AssetDatabase.LoadAssetAtPath(IOEditorHelper.FindFileByName(Application.dataPath, "vBugSpriteSheet"), typeof(Texture2D));
        } 
        #endregion
        //--------------------------------------- SPRITE SHEET --------------------------------------
        //--------------------------------------- SPRITE SHEET --------------------------------------
        ...
        public static void DrawWindowBGWaterMark(Rect windowRect) {
        ...
        private static void InitBGWaterMark() {
            if (bgWaterMark == null)
                bgWaterMark = (Texture2D)UnityEditor.AssetDatabase.LoadAssetAtPath(IOEditorHelper.FindFileByName(Application.dataPath, "vBugBGWatermark"), typeof(Texture2D));
        }
        #endregion
        //--------------------------------------- TILES BG WATERMARK --------------------------------------
        //--------------------------------------- TILES BG WATERMARK --------------------------------------
        ...
        public static Rect DrawXBoxControllerBG() {
            Texture2D xboxBG = GetXboxSpriteSheet();
            if (xboxBG == null)
                return default(Rect);

            Rect destRect = GUILayoutUtility.GetAspectRect((float)xboxBG.width / (float)xboxBG.height);
            Rect result = new Rect((destRect.width / 2f) - (xboxBG.width / 2f), (destRect.width / 2f) - (xboxBG.height / 2f), destRect.width, destRect.height);
            if (Event.current.type == EventType.repaint)
                GUI.DrawTextureWithTexCoords(result, xboxBG, GetExtCoords(result, xboxBGLoc));

            return result;
        }

        private static Rect GetExtCoords(Rect target, Rect source) {
            return new Rect(source.x / target.width, source.y / target.height, source.width / target.width, source.height / target.height);
        }

        private static Texture2D GetXboxSpriteSheet() {
            if (xboxSpriteSheet == null)
                xboxSpriteSheet = (Texture2D)UnityEditor.AssetDatabase.LoadAssetAtPath(IOEditorHelper.FindFileByName(Application.dataPath, "vBugXboxSpriteSheet"), typeof(Texture2D));

            return xboxSpriteSheet;
        }
