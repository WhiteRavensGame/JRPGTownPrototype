...
namespace Frankfort.VBug.Editor
{
    ...
    public class vBugConsoleWindow : vBugBaseWindow {
        ...
        private void RebuildCachedData() {
            ...
            int min = VerticalSliceDatabase.minRange;
            int max = VerticalSliceDatabase.maxRange;

            List<VerticalActivitySlice> availableLogs = new List<VerticalActivitySlice>();
            for (int i = min; i < max; i++) {
                VerticalActivitySlice slice = VerticalSliceDatabase.GetSlice(i);
                if (slice == null || slice.debugLogs == null || slice.debugLogs.calls == null || slice.debugLogs.calls.Length == 0)
                    continue;

                availableLogs.Add(slice);
            }
            RebuildCachedData(availableLogs.ToArray());
        }
        ...
        private void DrawGUI()
        {
            ...
            messagesScroll = GUILayout.BeginScrollView(messagesScroll, GUILayout.Height(dragBarRect.y));

            if (!VerticalSliceDatabase.isInitialized){
                EditorHelper.DrawNotInitializedLabel(this.position);
            } else {
                DrawAllAvailableLogs(messagesScroll.y);
            }
        ...
        private void DrawStackTrace() {
            ...
            if (!string.IsNullOrEmpty(selectedCall.stackTrace)) {
                ...
                stackTraceScroll = GUILayout.BeginScrollView(stackTraceScroll);
                string text = selectedCall.logString + "\n\n" + selectedCall.stackTrace;
                
                Vector2 size = EditorHelper.styleSelectableLabel.CalcSize(new GUIContent(text));
                EditorGUILayout.SelectableLabel(text, EditorHelper.styleSelectableLabel, GUILayout.Width(size.x), GUILayout.Height(size.y));
                GUILayout.EndScrollView();
            } else {
        ...
        private void FocusFrame(int newFrame)
        {
            ...
            for(int i = 0; i < cachedCallsData.Count; i++)
            {
                if(cachedCallsData[i].frameNumber >= newFrame)
                {
                    ...
                    messagesScroll.y = cachedCallsData[i].y;
                    return;
                }
            }
        }
