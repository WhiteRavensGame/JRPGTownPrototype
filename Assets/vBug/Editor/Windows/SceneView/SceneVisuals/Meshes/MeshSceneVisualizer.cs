...
namespace Frankfort.VBug.Editor {
    ...
    public class MeshSceneVisualizer : IBaseSceneVisualizer {
        ...
        private string GetMeshInfoByID(int frameNumber, int meshGOInstanceID) {
            long key = vBugMathHelper.CombineInts(frameNumber, meshGOInstanceID);
            ...
            if (infoPerFrame.ContainsKey(key)) {
                ...
                for (int i = frameNumber; i >= 0; i--) {
                    ...
                    string info =
                        "Mesh: " + mesh.header.meshName + "\n" +
                        "Parent: " + mesh.header.parentName + "\n" +
                        "Materials: " + (mesh.subMeshes != null ? mesh.subMeshes.materialIDs.Length.ToString() : "n/a") + "\n" +
                        "Triangles: " + (mesh.triangles != null ? (mesh.triangles.Length / 3).ToString() : "n/a") + "\n" +
                        "Vertices: " + (mesh.vertices != null ? mesh.vertices.count.ToString() : "n/a");
        ...
        private bool GetBoundsFromVisualizers(SWorldObject obj, int frameNumber, out Bounds result) {
            ...
            //--------------- From cache --------------------
            long key = vBugMathHelper.CombineInts(obj.instanceID, frameNumber);
