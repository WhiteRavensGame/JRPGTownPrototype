...
namespace Frankfort.VBug.Editor
{
    ...
    public class vBugTimelineWindow : vBugBaseWindow
    {
        ...
        private Dictionary<int, string> frameSceneName = new Dictionary<int, string>();
        ...
        private void DrawTimeLine() 
        {
            if (!VerticalSliceDatabase.isInitialized) {
                ...
                if (nonVisRange > 0) {
                    float scrollSize = Mathf.Max(300, nonVisRange / graphWidth);
                ...
                float newScaleX = EditorGUI.Slider(scaleSlider, graphScaleX, 0.04f, 4);
        ...
        private void DrawEventIndicators(Vector2 fpsPos, Vector2 memPos) {
            ...
            for (int i = startFrame; i < endFrame; i++) {
                ...
                if (frameHasConsoleLog.ContainsKey(i)) {
                    ...
                    frameSceneName.Add(i, slice.header.levelName);
                }
                //--------------- Caching --------------------


                //--------------- Detect level change --------------------
                if (frameSceneName.ContainsKey(i)) {
                    string currentLevelName = frameSceneName[i];
